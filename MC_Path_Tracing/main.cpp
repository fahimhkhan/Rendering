...
// -----------------------------------------
const int m_pixmap_width = 256, m_pixmap_height = 256;
const double m_samples_per_pixel = 1000;
...
m_Vector HemisphereSampling(m_Vector m_normal)
{
	...
	double Xi_1 = m_RND_1;
	double Xi_2 = m_RND_2;
	double r = sqrtf(1.0 - (Xi_1*Xi_1));
	double fi = 2.0*m_PI*Xi_2;


	double vx = (cos(fi)*r);// YOUR CODE HERE
	double vy = (sin(fi)*r); // YOUR CODE HERE
	double vz = Xi_1; // YOUR CODE HERE
...
void m_PathTracer(m_Ray &ray, int depth, m_Vector& color)
{
	...
	double ktot = (intersection.object->kd) + (intersection.object->ks) + (intersection.object->kr);// YOUR CODE HERE
	double m_random_float = GenerateRandomFloat(0.0, (float)ktot);// YOUR CODE HERE
	...
	if (m_random_float < intersection.object->kd) // send a diffuse ray
	{
		...
		ray.direction = HemisphereSampling(normal_at_hit_point);// YOUR CODE HERE
	...
	for (int col = 0; col < m_pixmap_width; col++)
	{
		...
		for (int row = 0; row < m_pixmap_height; row++)
		{
			...
			for (int s = 0; s < m_samples_per_pixel; s++)
			{
				...
				camera.x = camera.x + (m_RND_1 / m_pixmap_width);// YOUR CODE HERE
				camera.y = camera.y + (m_RND_1 / m_pixmap_height);// YOUR CODE HERE
